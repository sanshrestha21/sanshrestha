...
if [[ -z $1 ]]; then
    ...
    declare -a arr=(`ls -F . | egrep -v '/home/jack/rapping'`)
else
...
for i in ${arr[@]}; do
    ...
    rsync -av --delete /tmp/$i root@192.168.37.136:/path/to/backup/location/$i
    # remove our bare copy of the repository
    rm -rf /tmp/$i
done
# rsync -av --delete //where --delete can be omitted so all file can be copied 
# with out any extension conflict
